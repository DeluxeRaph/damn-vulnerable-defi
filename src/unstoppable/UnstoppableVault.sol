...
contract UnstoppableVault is IERC3156FlashLender, ReentrancyGuard, Owned, ERC4626, Pausable {
        ...
        returns (bool)
    {
        ...
        // @note Because this is a ERC4626 it has the converToChares()
        // 
        if (convertToShares(totalSupply) != balanceBefore) revert InvalidBalance(); // enforce ERC4626 requirement

        // transfer tokens out + execute callback on receiver
        // @note transfering token to user
        ERC20(_token).safeTransfer(address(receiver), amount);

        // callback must return magic value, otherwise assume it failed
        // @note this looks to be the fee needed to pay the loaner contract back
